...
def get_normalized_train_test_and_val_data():
    ...
    scaler = Normalizer().fit(filtered_sheet)
    rescaled_sheet = pd.DataFrame(scaler.fit_transform(filtered_sheet))
    rescaled_sheet = rescaled_sheet.rename(columns={0:'Unnamed: 0', 1:"cost_price", 2:'profit_markup',
...
if __name__ =="__main__":
    ...
    #####Подсчет количества всех вариаций по колонкам и вывод на гистограмму
    #sheet.hist(bins=50, figsize=(10,5))
    
    #####Вывод кареляций с пометкой цветом
    #correlation = filtered_sheet.corr().sort_values(by="profit",ascending=False)
    #sns.heatmap(correlation, vmax=1, square=True, annot= True) 
    #print(correlation)
    #plt.figure(figsize=(5,5))
    
    #####Вывод двумерного графика перебором столбцоцв
    #headers = ['profit', "depth_of_tree", "profit_markup"]
    #scatter_matrix(filtered_sheet, figsize=(12,12))
    ...
    #Подготовка датасета моделям
    

    #poly_features = PolynomialFeatures(degree=2)
    #x_poly = poly_features.fit_transform(x_train)
    #model.fit(x_poly, y_train)
    #y_poly_prediction = model.predict(x_poly)

    #rmse_poly = np.sqrt(mean_squared_error(y_train, y_poly_prediction))
    #r2_poly = r2_score(y_train, y_poly_prediction)
    #print("RMSE for poly regression", rmse_poly)
    #print("r2_score for poly regression", r2_poly)

    #test = x_train
    #test['y']=y_train
    #scatter_matrix(test)

    with open('main_output.txt', 'w') as output_file:
